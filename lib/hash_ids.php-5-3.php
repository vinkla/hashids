<?php
	...
	class hash_ids {
		...
		function encode() {
			return call_user_func_array(array($this, '_encode'), func_get_args());
		}
		...
		private function _encode() {
			...
			foreach ($args as $i => $arg) {
				...
				if ($i) {
					...
					$ret .= call_user_func_array(array($this, '_get_separator'), $params);
				}
		...
		private function _decode($hash) {
			...
			$validate = call_user_func_array(array($this, 'encode'), $ret);
			if ($validate != $hash)
				$ret = array();
